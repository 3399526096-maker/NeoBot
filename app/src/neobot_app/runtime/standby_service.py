...
class StandbyService:
    ...
    def set_hooks(
        ...
        *,
        on_enter: Any = _UNSET,
        on_resume: Any = _UNSET,
        on_onebot_change: Any = _UNSET,
    ) -> None:
        """注入生命周期动作（装配层在 core 建好运行时控制器后调用）。

        只覆盖传入的钩子，未传的保持原值；允许传入 None 显式清除。
        """
        if on_enter is not _UNSET:
            self._on_enter = on_enter
        if on_resume is not _UNSET:
            self._on_resume = on_resume
    ...
    async def resume(self, *, reason: str = "", operator: str = "") -> tuple[bool, str]:
        ...
        重建期间状态保持「待机」：入口循环据此判定「旧运行时退出不是进程退出」，
        并等待新运行时挂上后继续运行；重建失败时停在待机并记录失败原因，避免
